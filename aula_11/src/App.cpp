...
void App::searchMsg(std::string msg){
    std::vector<Mensagem*> found = diario.searchMsg(msg);

    if (found.empty()){
        std::cout << "Mensagem não encontrada..." << std::endl;
    }
    else{
        for (auto msgVc : found){
            std::cout << msgVc->date.toString() << ':' << msgVc->conteudo << std::endl;
        }
    }
}
