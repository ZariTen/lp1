...
bool cliente::compra(std::string nomeProduto,estabelecimento& e){
    ...
    for(int i = 0; i < e.produtos.size();++i){
        std::string name = e.produtos[i].nameProduct;
        ...
        if(name == nomeProduto){
            ...
            codigoProduto = e.produtos[i].codigo-1;
            produtoComprar = e.produtos[i];
            break;
        }
    }
    ...
    if(saldo < produtoComprar.preco){
        ...
        sacola.push_back(produtoComprar);
...
void cliente::verSacola(){
    for (int i = 0; i < sacola.size(); ++i){
        std::cout << sacola[i].nameProduct << " - R$" << sacola[i].preco << std::endl;
    }
}
...
void cliente::registro(){
    ...
    for (int i = 0; i < sacola.size(); ++i){
        arquivo << sacola[i].nameProduct << " - R$" << sacola[i].preco << std::endl;
    }
}